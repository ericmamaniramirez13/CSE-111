...
def compute_molar_mass(symbol_quantity_list, periodic_table):
    ...
    """for i in periodic_table:
            if x[0][0] in periodic_table:
                print("Asdasd")     
        print(periodic_table[x[0][0]])
        print(periodic_table[x[0][0]][1])
        y = 1+periodic_table[x[0][0]][1]
        print(y)"""
    product=0    
    for i in range (0, len (symbol_quantity_list)):
        product = product + periodic_table[symbol_quantity_list[i][0]][1] * symbol_quantity_list[i][1] 
    return product
def main():
    formula = input("Enter the molecula formula:  ")
    grams = float(input("Enter the mass in grams:  "))
    periodic_table = make_periodic_table()
    #print(parse_formula(formula, periodic_table))
    x = parse_formula(formula, periodic_table)
    molar_mass= compute_molar_mass(x, periodic_table)
    new_product = grams / molar_mass 
    print(f"\nMolar Mass: {molar_mass:.5f}\nGrams/Molar Mass = {grams}/{molar_mass:.5f}\n{new_product:.5f} Moles.\n")
